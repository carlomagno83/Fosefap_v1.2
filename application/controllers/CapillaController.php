...
require_once 'forms/Capilla.php';

class CapillaController extends Zend_Controller_Action
{
    ...
    {
        // action body
		$table = new Application_Model_DbTable_Capilla(); 
    ...
    public function addAction()
    {
        ...
        $this->view->title = "Agregar Capilla";
        //valor para <head><title>
        $this->view->headTitle($this->view->title);
        //creo el formulario
        $form = new Application_Form_Capilla();
        //cambio el texto del boton submit
        $form->submit->setLabel('Agregar Capilla');
        ...
        if ($this->getRequest()->isPost())
        {
            ...
            if ($form->isValid($formData))
            {
				...
				$idataud = $form->getValue('idataud'); 				
				$idcandelabros = $form->getValue('idcandelabros'); 				
				$manto = $form->getValue('manto'); 
				$cristo = $form->getValue('cristo'); 
				$velas = $form->getValue('velas'); 
				$flamas = $form->getValue('flamas'); 
				$alfombra = $form->getValue('alfombra'); 
				$letrero = $form->getValue('letrero'); 
				$tarjetero = $form->getValue('tarjetero'); 
				$observacion = $form->getValue('observacion'); 				
										                              
                //creo objeto Album que controla la talba Album de la base de datos
                $capilla = new Application_Model_DbTable_Capilla();
                //llamo a la funcion agregar, con los datos que recibi del form
                			
				$capilla->agregar($idataud,$idcandelabros,$manto,$cristo,$velas,$flamas,$alfombra,$letrero,$tarjetero,$observacion);
    ...
    public function deleteAction()
    {
        ...
        $tabla = new Application_Model_DbTable_Capilla();
    ...
    public function updateAction()
    {
        ...
        if ($this->getRequest()->isPost())
        {
            ...
            if ($form->isValid($formData))
            {
                //aca ya estamos seguros de que los datos son validos
                //ahora los extraemos como se ve abajo
				$idcapilla = $form->getValue('idcapilla'); 
				$idataud = $form->getValue('idataud'); 				
				$idcandelabros = $form->getValue('idcandelabros'); 				
				$manto = $form->getValue('manto'); 
				$cristo = $form->getValue('cristo'); 
				...
				$capilla->cambiar($idcapilla,$idataud,$idcandelabros,$manto,$cristo,$velas,$flamas,$alfombra,$letrero,$tarjetero,$observacion);
                //indico que despues de haber agregado el album,
                //me redirija a la accion index de AlbumController, es decir,
                //a la pagina que me muestra el listado de albumes
                $this->_helper->redirector('index');
