...
require_once 'models/DbTable/Cementerio.php';

class Application_Form_Cementerio extends Zend_Form
{

    public function init()
    {
        /* Form Elements & Other Definitions Here ... */
		$this->setName('Cementerio');

        //campo hidden para guardar id del cementerio
        $idcementerio = new Zend_Form_Element_Hidden('idcementerio'); 
        $idcementerio->addFilter('Int');
        ...
        $nombre->setLabel('Nombre del cementerio:')->setRequired(true)->addFilter('StripTags')->addFilter('StringTrim')->
                 addValidator('NotEmpty');
		 
		 
        //creamos <input text> para escribir direccion del cementerio
        $direccion = new Zend_Form_Element_Text('direccion');
        $direccion->setLabel('Direccion del cementerio:')->setRequired(true)->addFilter('StripTags')->addFilter('StringTrim')->
                 addValidator('NotEmpty');
		...
		*/
        ...
        $telefono_fijo->setLabel('Telefono del cementerio:')->setRequired(true)->addFilter('StripTags')->addFilter('StringTrim')->
                 addValidator('NotEmpty');
				 
		//creamos <input text> para escribir celular del cementerio
        $telefono_celular = new Zend_Form_Element_Text('telefono_celular');
        $telefono_celular->setLabel('Celular del cementerio:')->setRequired(true)->addFilter('StripTags')->addFilter('StringTrim')->
                 addValidator('NotEmpty');
				 
		//creamos <input text> para escribir correo electronico del cementerio
        $mail = new Zend_Form_Element_Text('mail');
        $mail->setLabel('Correo Electronico del cementerio:')->setRequired(true)->addFilter('StripTags')->addFilter('StringTrim')->
                 addValidator('NotEmpty');
		
		//creamos <input text> para escribir costo alquiler del cementerio
        $costo = new Zend_Form_Element_Text('costo');
        $costo->setLabel('Costo Alquiler del cementerio:')->setRequired(true)->addFilter('StripTags')->addFilter('StringTrim')->
                 addValidator('NotEmpty');
		...
		$this->addElements(array($idcementerio,$RUC,$nombre,$direccion,$iddepartamento,$idprovincias,$iddistrito,$telefono_fijo,$telefono_celular,$mail,$costo,$observacion,$submit));	
		/* Form Elements & Other Definitions Here ... */
