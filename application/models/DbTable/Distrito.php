...
class Application_Model_DbTable_Distrito extends Zend_Db_Table_Abstract
{
    ...
    protected $_sql = 'SELECT iddistrito,nombre,idprovincia FROM distritos';    
	...
	public function listarFiltroDistritos($codigo){
        ...
        public function countDistritos()
        {

            $db = Zend_Registry::get('db');	
            $stmt = $db->query("SELECT COUNT(iddistrito) ndistr FROM distritos");     
            $row = $stmt->fetch();
            return $row['ndistr'];		
